...
`/kevinle128-skills:implement`/`/kevinle128-skills:git` handoff): pass `--advice` to the downstream skill so
...
When `--html` is present, activate `/kevinle128-skills:frontend-design` before composing the
HTML artifact. If `kevinle128-skills:frontend-design` requires design intelligence, follow its
`kevinle128-skills:ui-ux-pro-max` activation rule before styling.
...
- If another workflow requires `plan.md` (for example `--github`), keep
  ...
  full plan body unless a downstream `/kevinle128-skills:implement` handoff explicitly needs it.
...
- For `--html`, keep the self-contained `plan.html` local by default and report
  ...
  agentwiki sites upload "{plan-dir}/plan.html" \
    --description "{plan title} - kevinle128-skills:plan HTML artifact" \
    --auto-summary
  ```
...
flowchart TD
    ...
    L --> X
    X -->|Yes| Y[Activate kevinle128-skills:frontend-design<br/>Write plan.html]
...
6. **Red Team Review** → Run `/kevinle128-skills:plan red-team {plan-path}` (hard/deep/parallel/two modes)
7. **Post-Plan Validation** → Run `/kevinle128-skills:validate-plan {plan-path}` (hard/deep/parallel/two modes)
8. **HTML Artifact** → If `--html`, activate `/kevinle128-skills:frontend-design` and write final reviewed `plan.html` as the primary output
...
13. **Journal** → Run `/kevinle128-skills:journal` to write a concise technical journal entry upon completion. See the shared "Journal step — opt-out" block below.
...
This gate is mandatory after `/kevinle128-skills:validate-plan` or `/kevinle128-skills:plan red-team` edits any plan file.
Load: `../kk-validate-plan/references/verification-roles.md` → "Whole-Plan Consistency Sweep".

Before recommending `/kevinle128-skills:implement`, re-read `plan.md` and every `phase-*.md` file. Search all plan files for stale terms, rejected assumptions, renamed APIs/files/fields, superseded decisions, and duplicate embedded drafts/contracts. Reconcile contradictions across the entire plan, not only the edited phase.
...
## Output Requirements
**IMPORTANT:** Invoke "/kevinle128-skills:project-organization" skill to organize the outputs.
...
that `ak plan resolve`/`kevinle128-skills:implement` use across sessions without any GitHub link).
...
|------------|-----------|---------|
| `/kevinle128-skills:plan archive` | `references/archive-workflow.md` | Archive plans + write journal entries |
| `/kevinle128-skills:plan red-team` | `references/red-team-workflow.md` | Adversarial plan review with hostile reviewers |
...
|--------|----------------|-----|
| `/kevinle128-skills:validate-plan <plan-path>` | Plan is moderate-to-complex; user wants critical-questions interview before implementation | Cheapest gate — surfaces unspecified assumptions, missing acceptance criteria, hand-wavy phases |
| `/kevinle128-skills:plan red-team` | Plan touches security, auth, payments, data integrity, public APIs, infra, or has high blast radius | Adversarial reviewers stress-test the plan for failure modes, attack vectors, and missing edge cases |
| `/kevinle128-skills:implement <plan-path>` | Plan is small / well-understood / low-risk and user wants to start implementation | Skip extra gates; go straight to implementation |
...
- Omit `/kevinle128-skills:plan red-team` from the offered options when mode is `--hard`, `--deep`, `--parallel`, or `--two` (Step 6 already ran adversarial review).
- Omit `/kevinle128-skills:validate-plan` from the offered options when mode is `--deep` (Step 7 already ran validation).
- If both gates already ran, the Post-Plan Handoff still fires but offers only `/kevinle128-skills:implement <plan-path>` and `End session`.
...
**Typically follows:** `/kevinle128-skills:brainstorm` (after exploring options), `/kevinle128-skills:scout` (after codebase discovery)
**May precede:** `/kevinle128-skills:implement` after user approval (otherwise stop with plan path and next-step options)
**Related:** `/kevinle128-skills:brainstorm` (explore before planning), `/kevinle128-skills:validate-plan` (verify an existing plan), `/kevinle128-skills:implement` (execute after planning)
