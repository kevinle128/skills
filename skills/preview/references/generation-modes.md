...
When generating mermaid code blocks, use `/kevinle128-skills:mermaidjs-v11` skill for v11 syntax rules.
...
For full SVG layout rules (spacing minimums, arrow connection points, z-index ordering, anti-pattern catalog), read `/kevinle128-skills:tech-graph`'s `references/svg-layout-best-practices.md`. For full publish-grade SVG+PNG diagram generation (7 visual styles, agent/memory primitives), use `/kevinle128-skills:tech-graph` directly.
...
For `--slides`: recommend invoking `/kevinle128-skills:ui-ux-pro-max` for richer style selection.
Must use `/kevinle128-skills:mermaidjs-v11` for any Mermaid diagrams.
