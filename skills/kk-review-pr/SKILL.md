...
- **Merge** (`--merge`): after all other modes complete, if the PR is ready to merge, activate `kk:git merge-pr` to merge it, watch post-merge CI until green, and verify follow-up before stopping.
...
- **When the `--fix` loop is stuck** — same finding survives 3 attempts,
  `kk:fix` is blocked, or CI keeps reding for the same reason; pass every
...
Activate `kk:fix --auto` with the full findings list and PR context:

```
kk:fix --auto "Fix all actionable findings from review-pr <PR_REF>: <finding summary>"
```
...
`kk:fix` performs its own scout, diagnose, implementation, verification, and prevention flow. Do not bypass its hard gates.

### 3. Commit and push

After `kk:fix` verifies the fixes, activate:

```
kk:git cp
```

This stages, commits, and pushes the fixes to the PR head branch. Do not run `kk:git cp` if verification failed, secrets are detected, or the working tree contains unrelated user changes.
...
When `--advice` is originally set and the loop stalls (same finding survives 3 attempts, `kk:fix` blocked, CI unresolvable), spawn `kongming` at the "loop is stuck" checkpoint before declaring the stop condition — see Advisory supervision.

Stop only when one of:
- the re-review finds no actionable findings
- `kk:fix` is blocked by a missing user/business decision
- the same finding survives 3 consecutive fix attempts (loop not converging)
- CI or local verification fails in a way `kk:fix` cannot resolve without user input
...
If the loop terminates due to a blocker (non-converging, `kk:fix` blocked, CI unresolvable), still post the final review — but the verdict will reflect remaining findings (likely **Request changes** or **Comment**), and the body should include the blocker so the human reviewer knows where to take over.
...
Activate `kk:git merge-pr` with the PR reference:

```
kk:git merge-pr <PR_REF>
```

`kk:git merge-pr` (documented in the `kk:git` skill) owns the mechanics:
...
- on deterministic CI failure, drives a follow-up fix (`kk:fix --auto` on a new branch) and repeats, up to 3 attempts
...
- If `kk:git merge-pr` refuses (gate failure, branch protection, conflicts): report the blocker; do not retry with different flags to force the merge.
