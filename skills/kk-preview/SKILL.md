...
### View Mode
- `/kk:preview <file.md>` - View markdown file in novel-reader UI
- `/kk:preview <directory/>` - Browse directory contents
- `/kk:preview --stop` - Stop running server

### Generation Mode (Markdown)
- `/kk:preview --explain <topic>` - Generate visual explanation (ASCII + Mermaid + prose)
- `/kk:preview --slides <topic>` - Generate presentation slides (one concept per slide)
- `/kk:preview --diagram <topic>` - Generate focused diagram (ASCII + Mermaid)
- `/kk:preview --ascii <topic>` - Generate ASCII-only diagram (terminal-friendly)

### Generation Mode (HTML)
- `/kk:preview --html --explain <topic>` - Self-contained HTML explanation
- `/kk:preview --html --slides <topic>` - Magazine-quality HTML slide deck
- `/kk:preview --html --diagram <topic>` - HTML diagram with zoom controls
- `/kk:preview --html --diff [ref]` - Visual diff review
- `/kk:preview --html --plan-review [plan-file]` - Plan vs codebase comparison
- `/kk:preview --html --recap [timeframe]` - Project context snapshot
...
| Flag without topic | Ask user: "Please provide a topic: `/kk:preview --explain <topic>`" |
...
| Server startup failure | Check if port in use, try `/kk:preview --stop` first |
...
Use `/kk:mermaidjs-v11` skill for Mermaid syntax validation.
...
- For `--slides`: consider invoking `/kk:ui-ux-pro-max` for richer style selection
