...
If `/kk:ai-multimodal` skill is available and image generation is appropriate, it can be used for hero banners, conceptual illustrations, and decorative accents that establish the page's visual tone.
...
For `--slides` presentation-grade output, consider invoking `/kk:ui-ux-pro-max` for richer style selection and distinctive font/palette pairing.
