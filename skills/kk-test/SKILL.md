...
Browser-based visual testing via `kk:agent-browser`, `kk:chrome-profile`, `kk:web-testing`, or project-native Playwright/Vitest/k6 commands. Covers screenshots, responsive checks, accessibility audits, form automation, and console error collection.
...
6. For frontend: run UI tests via `kk:agent-browser`, `kk:chrome-profile`, `kk:web-testing`, or project-native browser tests
...
- **Browser**: `kk:agent-browser` for live browser interaction without real user cookies; `kk:chrome-profile` for the user's actual Chrome login state, opened with `chrome-profile open --json` and bound by the returned selector; `kk:web-testing` or project-native Playwright/Vitest/k6 for repeatable UI tests
- **Analysis**: `kk:ai-multimodal` skill for screenshot analysis
- **Debugging**: `kk:debug` skill when tests reveal bugs requiring investigation
- **Thinking**: `kk:sequential-thinking` skill for complex test failure analysis
...
**Typically follows:** `/kk:implement` (test after implementation), `/kk:fix` (test after bug fix)
**Typically precedes:** `the installed code-review skill` (review after tests pass)
**Related:** `/kk:implement` (implement then test), `/kk:fix` (fix then test)
