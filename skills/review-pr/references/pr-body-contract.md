...
Pull request bodies must contain these sections.
Headings must use the effective writing language; English forms are shown.
Validate with:
...
Retain these fields when they are relevant, without duplicating facts:
...
- Target branch and delivery mode

## `kevinle128-skills:review-pr` validation

- Missing required sections → **Important** findings.
- Unsupported claims / empty evidence where evidence is asserted → **Important**.
- Missing linked issues or target branch details when the PR claims them → **Suggestion**.
