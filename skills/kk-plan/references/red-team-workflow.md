...
3. If no plan found → Ask user to specify path or run `/kk:plan` first
...
## Next Steps
Remind the user they can run `/kk:validate-plan {plan-directory-path}` before implementation.
When the user approves implementation, run `/kk:implement {ABSOLUTE_PATH_TO_PLAN_DIR}/plan.md`.
