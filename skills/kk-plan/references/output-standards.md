...
- Name the code owner, registration or routing change, contract, and wiring for every new runtime path segment
- Include an end-to-end test that enters through the same runtime boundary and proves the connected behavior
- Keep internal feature owners real in the end-to-end test and mock only third-party systems at their boundary

### Runtime Flow Proof Matrix

Every full `plan.md` must contain one row for each distinct promised feature or acceptance behavior.
...
A plan is ready only when every row and the gate are `PASSED`.
Repair `FAILED` rows in the plan.
Resolve `NEEDS_DECISION` rows through popup HITL before readiness.
...
- Prove every promised feature through its complete runtime flow and same-boundary end-to-end test
