...
argument-hint: "[task] [--fast|--hard|--deep|--parallel|--two] [--tdd|--no-tasks] [--html] [--github] [--wiki] [--advice] [--yagni] [--skip-journal] OR [archive|red-team]"
license: MIT
metadata:
  author: KevinKit
  version: "1.5.0"
...
GitHub remote still has a fully working plan as files. Full model: `../kk-implement/references/plan-state-files-first.md`.
...
- Internal path: ...
- Prepared data and external mocks: ...
- Observable result: ...
- End-to-end proof: ...
...
`/kk:implement`/`/kk:git` handoff): pass `--advice` to the downstream skill so
...
When `--html` is present, activate `/kk:frontend-design` before composing the
HTML artifact. If `kk:frontend-design` requires design intelligence, follow its
`kk:ui-ux-pro-max` activation rule before styling.
...
- If another workflow requires `plan.md` (for example `--github`), keep
  ...
  full plan body unless a downstream `/kk:implement` handoff explicitly needs it.
...
owned by ak-vibe/kk-issue-to-plan; `ready to review` marks a
...
- For `--html`, keep the self-contained `plan.html` local by default and report
  ...
  agentwiki sites upload "{plan-dir}/plan.html" \
    --description "{plan title} - kk:plan HTML artifact" \
    --auto-summary
  ```
...
### 6. Runtime Flow Proof
Load: `../kk-validate-plan/references/verification-roles.md` → "Mandatory Runtime Flow Proof Gate".
...
flowchart TD
    ...
    F --> G
    G --> H[Write Draft Plan via Planner]
    H --> T[Runtime Flow Proof Gate]
    T -->|NEEDS_DECISION| U[HITL popup]
    U --> H
    T -->|FAILED| Z[Repair Plan]
    Z --> T
    T -->|PASSED| I{Red Team?}
    ...
    L --> X
    X -->|Yes| Y[Activate kk:frontend-design<br/>Write plan.html]
...
5. **Plan Documentation** → Write a comprehensive draft plan via planner subagent
6. **Runtime Flow Proof** → Prove every promised feature through a real runtime boundary and same-boundary end-to-end test; repair `FAILED` rows and resolve every `NEEDS_DECISION` row through HITL
7. **Red Team Review** → Run `/kk:plan red-team {plan-path}` (hard/deep/parallel/two modes)
8. **Post-Plan Validation** → Run `/kk:validate-plan {plan-path}` (hard/deep/parallel/two modes)
9. **HTML Artifact** → If `--html`, activate `/kk:frontend-design` and write final reviewed `plan.html` as the primary output
10. **Hydrate Progress** → Mirror phases into live task management when available (default on, `--no-tasks` to skip)
11. **GitHub Issue** → If `--github`, create/update issue and apply `ready to review`
12. **AgentWiki Publish** → If `--wiki`, publish final docs privately or upload `plan.html` only when AgentWiki CLI/MCP is available and the requested visibility permits it
13. **Boundary Reminder** → Present optional next-step commands with absolute path
14. **Journal** → Run `/kk:journal` to write a concise technical journal entry upon completion. See the shared "Journal step — opt-out" block below.

### Mandatory Runtime Flow Proof Gate

This gate runs in every planning mode, including `--fast`, before red-team review, optional validation, task hydration, publication, or implementation handoff.
In this contract, runtime means the real operational path by which a user, system, scheduler, event source, or owning caller invokes the behavior.
It does not mean a deployed production environment.

1. Extract every distinct promised feature and acceptance behavior.
2. For each feature, record the actor, real or explicitly planned trigger, registered entry point, ordered internal path, prepared data, permitted external mocks, and observable result.
3. Verify existing path segments against source.
4. Give every new segment a code-level implementation task that names its owner, registration or routing change, contract, and wiring.
5. Plan an end-to-end test that enters through the same runtime boundary, keeps internal owners real, and mocks only third-party systems at their boundary.
6. Record the result in the `Runtime Flow Proof Matrix` in `plan.md`.

When the requested work copies, ports, or claims parity with another system, inspect that system's actual trigger, registration or discovery, internal path, lifecycle operations in scope, and observed behavior.
Names, folder layout, package metadata, and documentation alone are not runtime proof.

If a row is `FAILED`, repair the plan and rerun the row.
If a required trigger or public/system contract is missing, conflicting, or materially ambiguous, mark the row `NEEDS_DECISION` and open a popup-capable `ask_user capability` call.
Offer two or three codebase-grounded options, put the recommended option first, and explain the runtime owner and end-to-end test created by each option.
Do not continue to readiness until the user answers and every row is `PASSED`.

### Journal step — opt-out

Skip the automatic `/kk:journal` step when either applies:
...
Explicit `/kk:journal` and `ak journal create` are unaffected.

### Whole-Plan Consistency Gate

This gate is mandatory after `/kk:validate-plan` or `/kk:plan red-team` edits any plan file.
Load: `../kk-validate-plan/references/verification-roles.md` → "Whole-Plan Consistency Sweep".

Before recommending `/kk:implement`, re-read `plan.md` and every `phase-*.md` file. Search all plan files for stale terms, rejected assumptions, renamed APIs/files/fields, superseded decisions, and duplicate embedded drafts/contracts. Reconcile contradictions across the entire plan, not only the edited phase.
...
## Output Requirements
**IMPORTANT:** Invoke "/kk:project-organization" skill to organize the outputs.
...
that `ak plan resolve`/`kk:implement` use across sessions without any GitHub link).
...
|------------|-----------|---------|
| `/kk:plan archive` | `references/archive-workflow.md` | Archive plans + write journal entries |
| `/kk:plan red-team` | `references/red-team-workflow.md` | Adversarial plan review with hostile reviewers |
...
|--------|----------------|-----|
| `/kk:validate-plan <plan-path>` | Plan is moderate-to-complex; user wants critical-questions interview before implementation | Cheapest gate — surfaces unspecified assumptions, missing acceptance criteria, hand-wavy phases |
| `/kk:plan red-team` | Plan touches security, auth, payments, data integrity, public APIs, infra, or has high blast radius | Adversarial reviewers stress-test the plan for failure modes, attack vectors, and missing edge cases |
| `/kk:implement <plan-path>` | Plan is small / well-understood / low-risk and user wants to start implementation | Skip extra gates; go straight to implementation |
| End session | User wants to review/share plan before deciding | Stop with plan path returned |

**Skip this step ONLY when:**
- The current invocation IS already a subcommand (`red-team` or `archive`) — those have their own terminal handoff.
...
- Omit `/kk:plan red-team` from the offered options when mode is `--hard`, `--deep`, `--parallel`, or `--two` (Step 7 already ran adversarial review).
- Omit `/kk:validate-plan` from the offered options when mode is `--deep` (Step 8 already ran validation).
- If both gates already ran, the Post-Plan Handoff still fires but offers only `/kk:implement <plan-path>` and `End session`.
...
**Typically follows:** `/kk:brainstorm` (after exploring options), `/kk:scout` (after codebase discovery)
**May precede:** `/kk:implement` after user approval (otherwise stop with plan path and next-step options)
**Related:** `/kk:brainstorm` (explore before planning), `/kk:validate-plan` (verify an existing plan), `/kk:implement` (execute after planning)
