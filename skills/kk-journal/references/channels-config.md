...
channels:
  - id: <string, unique>            # referenced by --channels and posted.json
    ...
    build_in_public: <bool, optional>  # tag for kk-ship --social targeting

groups:
  build_in_public: [<channel-id>, ...]  # optional named group, e.g. used by kk-ship --social
```
...
| `build_in_public` | no | Informational tag; `groups.build_in_public` is what `kk-ship --social` actually reads. |
