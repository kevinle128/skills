...
**Typically follows:** `/kk:implement` (journal after implementation), `/kk:fix` (journal after bug fix), or `/kk:git` (journal after delivery)
...
Explicit `/kk:journal` and `ak journal create` are always available and are
unaffected by any preference or flag.

The **automatic** journal step at the end of the `kk:plan`, `/kk:implement`,
`/kk:fix`, and `kk:bootstrap` skills honors:
