...
test('loadEnv: missing files across the whole cascade returns empty object', () => {
  const projectRoot = makeTempDir('kk-journal-env-missing-');
  const homeDir = makeTempDir('kk-journal-env-missing-home-');
...
test('loadEnv: cascade precedence — project .agentkit/.env wins over user .agentkit/.env, .claude/.env layers, but process.env still wins via resolveEnv', () => {
  const projectRoot = makeTempDir('kk-journal-env-cascade-');
  const homeDir = makeTempDir('kk-journal-env-cascade-home-');
  const originalHome = process.env.HOME;
  try {
    process.env.HOME = homeDir;
...
test('resolveEnv: falls back to defaultValue when key is absent everywhere', () => {
  const projectRoot = makeTempDir('kk-journal-env-default-');
  const homeDir = makeTempDir('kk-journal-env-default-home-');
  const originalHome = process.env.HOME;
  try {
    process.env.HOME = homeDir;
...
test('resolveAllEnv: merges file cascade with process.env, process.env winning', () => {
  const projectRoot = makeTempDir('kk-journal-env-merge-');
  const homeDir = makeTempDir('kk-journal-env-merge-home-');
  const originalHome = process.env.HOME;
  try {
    process.env.HOME = homeDir;
    fs.mkdirSync(path.join(projectRoot, '.agentkit'), { recursive: true });
