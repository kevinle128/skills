...
function makeProject() {
  const projectRoot = makeTmpDir('kk-journal-generate-video-project-');
  fs.mkdirSync(path.join(projectRoot, '.agentkit'), { recursive: true });
  return projectRoot;
}

function makeSkillsRootWithHyperframes() {
  const root = makeTmpDir('kk-journal-generate-video-skills-');
...
test('generate-video: --video <path> returns array of resolved paths', async () => {
  const fixturesDir = makeTmpDir('kk-journal-generate-video-fixtures-');
  try {
...
test('generate-video: engine auto-detect falls back to remotion when hyperframes is absent', () => {
  const skillsRoot = makeTmpDir('kk-journal-generate-video-skills-empty-');
  try {
    const engine = resolveEngine({ configuredEngine: 'auto', env: { MOCK_INSTALLED_SKILLS_ROOT: skillsRoot } });
  ...
  try {
    ...
    const result = await generateVideos({
      ...
      env: {
        ...
        MOCK_INSTALLED_SKILLS_ROOT: makeTmpDir('kk-journal-generate-video-skills-none-'),
      },
    });
...
test('generate-video: cache hit — same key returns cached path without regeneration', async () => {
  ...
  try {
    ...
    const mockOutput = writeMockSource(fixturesDir);
    const env = { ...process.env, MOCK_MULTIX_OUTPUT: mockOutput, MOCK_INSTALLED_SKILLS_ROOT: makeTmpDir('kk-journal-generate-video-skills-x-') };
...
test('generate-video: --dry-run skips generation entirely (no file written, mock untouched)', async () => {
  ...
  try {
    ...
    const result = await generateVideos({
      ...
      cwd: projectRoot,
      dryRun: true,
      env: { ...process.env, MOCK_MULTIX_OUTPUT: mockOutput, MOCK_INSTALLED_SKILLS_ROOT: makeTmpDir('kk-journal-generate-video-skills-dry-') },
    });
