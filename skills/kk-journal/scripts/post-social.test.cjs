...
function makeFakeNpxDir() {
  const binDir = fs.mkdtempSync(path.join(os.tmpdir(), 'kk-journal-fake-npx-'));
...
function makeProject() {
  const projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'kk-journal-post-social-'));
...
function runCli(args, { env = {}, cwd } = {}) {
  const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'kk-journal-post-social-home-'));
  try {
...
test('post-social: missing ZERNIO_API_KEY produces a clear error and exits 1 without invoking the CLI', () => {
  ...
  try {
    ...
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'kk-journal-post-social-nohome-'));
    try {
...
test('post-social: live path — the zernio child only sees the scoped env (ZERNIO_* + npx-resolution vars), not unrelated secrets', () => {
  const projectRoot = makeProject();
  const binDir = makeFakeNpxDir();
  const envDumpPath = path.join(os.tmpdir(), `kk-journal-zernio-env-dump-${process.pid}-${Date.now()}.json`);
  try {
    writeJournalYaml(projectRoot, 'channels:\n  - id: x_main\n    platform: x\n    account_id: acc_x_123\n');
    const journalFile = writeJournalFile(projectRoot, 'Body.');
