...
Not for: full project cloning (`kevinle128-skills:bootstrap`), simple file copy, or package installation.

## Usage

```text
/kevinle128-skills:feature-lens <github-url|owner/repo|local-path> [feature-description] [--compare|--copy|--improve|--port] [--auto|--fast]
```
...
1. Pack the source with `/kevinle128-skills:repomix`.
...
4. Use `/kevinle128-skills:scout` on the local project to map architecture, similar features, and integration points.
...
- activate `/kevinle128-skills:sequential-thinking` to trace multi-step flows
...
Do not invoke `/kevinle128-skills:brainstorm` from inside `feature-lens`; that skill can create its own planning handoff and break `feature-lens` phase ownership.
...
Delegate to `/kevinle128-skills:plan` with:
- source manifest
- the source anatomy
- dependency matrix
...
- `feature-lens` is a front door, not a second orchestration stack. Keep planning and delivery ownership in `kevinle128-skills:plan` and `kevinle128-skills:implement`.
...
- other modes: present the plan path and hand implementation to `/kevinle128-skills:implement`

Implementation handoff text:

```text
Plan ready at ./plans/<plan-dir>/plan.md. To implement, run /kevinle128-skills:implement <plan-path>.
```
