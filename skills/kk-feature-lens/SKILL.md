...
Not for: full project cloning (`kk:bootstrap`), simple file copy, or package installation.

## Usage

```text
/kk:feature-lens <github-url|owner/repo|local-path> [feature-description] [--compare|--copy|--improve|--port] [--auto|--fast]
```
...
1. Pack the source with `/kk:repomix`.
...
4. Use `/kk:scout` on the local project to map architecture, similar features, and integration points.
...
- activate `/kk:sequential-thinking` to trace multi-step flows
...
Do not invoke `/kk:brainstorm` from inside `feature-lens`; that skill can create its own planning handoff and break `feature-lens` phase ownership.
...
- `feature-lens` is a front door, not a second orchestration stack. Keep planning and delivery ownership in `kk:plan` and `kk:implement`.
...
- other modes: present the plan path and hand implementation to `/kk:implement`

Implementation handoff text:

```text
Plan ready at ./plans/<plan-dir>/plan.md. To implement, run /kk:implement <plan-path>.
```
