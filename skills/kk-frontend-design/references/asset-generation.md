...
Complete workflow for generating design-aligned visual assets using `kk:ai-multimodal` skill.
...
Use `kk:ai-multimodal` skill's image generation capabilities:
...
Use `kk:ai-multimodal` to analyze the generated asset (see `visual-analysis.md` for complete workflow):
...
4. Consider using `kk:media-processing` skill for post-generation adjustments
