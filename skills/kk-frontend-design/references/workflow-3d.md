...
## Prerequisites
- Activate `kk:ui-ux-pro-max` skill first
- Activate `kk:threejs` skill for 3D and WebGL expertise
- Have `kk:ai-multimodal` skill ready for asset generation

## Initial Research
Run `kk:ui-ux-pro-max` searches:
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/kk-ui-ux-pro-max/scripts/search.py "<product-type>" --domain product
python3 ${CLAUDE_PLUGIN_ROOT}/skills/kk-ui-ux-pro-max/scripts/search.py "immersive 3d" --domain style
python3 ${CLAUDE_PLUGIN_ROOT}/skills/kk-ui-ux-pro-max/scripts/search.py "animation" --domain ux
```
...
Use `kk:ai-multimodal` skill for:
- Textures and materials
- Skyboxes and environment maps
- Particle sprites
- Video backgrounds

Use `kk:media-processing` skill for:
