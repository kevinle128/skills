...
Entry point for using the `kk:ai-multimodal` skill to generate and analyze visual assets that align with frontend design thinking and aesthetic guidelines.

## When to Use

Use `kk:ai-multimodal` in frontend design when you need to:
...
Use `aesthetic` for overall design system guidance and quality evaluation framework. Then use `kk:frontend-design` with `kk:ai-multimodal` for asset generation and analysis that follows those guidelines.

### With `kk:agent-browser`
Use `kk:agent-browser` to capture screenshots from inspiration websites for design extraction. Capture at actual viewport size, not full-page scrolls.
...
### With `kk:media-processing` Skill
