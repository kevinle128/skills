...
**Typically precedes:** `the engineer debug skill` (debug after scouting), `/kk:fix` (fix after locating code), `the installed code-review skill` (scout edge cases before review)
**Related:** `the engineer debug skill` (investigate after scouting), `/kk:brainstorm` (explore after scouting)
