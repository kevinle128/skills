...
Visual verification of frontend implementations via `kevinle128-skills:agent-browser`, `kevinle128-skills:chrome-profile`, Chrome MCP / `chrome-devtools-mcp`, or project-native browser tests. Use `kevinle128-skills:chrome-profile` and its exact tab binding when real Chrome profile state matters; raw Chrome MCP navigation is only for generic/profile-independent inspection. Detect if frontend-related -> check browser tool availability -> screenshot + console error check -> report. Skip if not frontend.
...
- **Codebase:** `kevinle128-skills:docs-seeker` skill for package/plugin docs; `kevinle128-skills:repomix` skill for codebase summary
- **Scouting:** `/kevinle128-skills:scout` or `/kevinle128-skills:scout ext` for finding relevant files
- **Frontend:** `kevinle128-skills:agent-browser`, `kevinle128-skills:chrome-profile`, Chrome MCP / `chrome-devtools-mcp`, or project-native browser tests for visual verification. For real profile state, let `chrome-profile open --json` create the tab and bind to its returned selector before using MCP inspection tools.
- **Skills:** Activate `kevinle128-skills:problem-solving` skill when stuck on complex issues
...
**Typically follows:** `/kevinle128-skills:scout` (after locating relevant code)
**Typically precedes:** `/kevinle128-skills:fix` (fix the diagnosed issue), `/kevinle128-skills:brainstorm` (explore solutions for complex problems)
**Related:** `/kevinle128-skills:scout` (discover before debugging), `/kevinle128-skills:fix` (fix after diagnosing)
