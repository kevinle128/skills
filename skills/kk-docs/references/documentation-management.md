...
full operations, invoke `/kk:docs init`, `/kk:docs update`, `/kk:docs summarize`,
or `/kk:docs agent-context` (root `CLAUDE.md`/`AGENTS.md`).
...
- `/kk:implement` and `/kk:fix`: update docs during finalize only when the impact
  criteria above apply.
- The installed planning skill reads routed project context before creating an
  execution plan.
- `/kk:preview`: keep temporary visuals with the active plan; link them from
  evergreen docs only when they remain useful.
- `/kk:tech-graph`: use for publish-grade diagrams when a visual materially
