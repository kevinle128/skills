...
This is not the default for ordinary browser testing. Use `kk:agent-browser` when a fresh or tool-managed browser is enough. Use project-native Playwright or `kk:web-testing` for repeatable CI tests. Use this skill when the user's actual Chrome profile state matters.
...
bash ~/.claude/skills/kk-chrome-profile/scripts/install.sh
bash .claude/skills/kk-chrome-profile/scripts/install.sh
...
bash ~/.claude/plugins/<plugin>/skills/kk-chrome-profile/scripts/install.sh

# AgentKit / Codex native-skill layout, global or project-local
bash ~/.agents/skills/kk-chrome-profile/scripts/install.sh
bash .agents/skills/kk-chrome-profile/scripts/install.sh
```
