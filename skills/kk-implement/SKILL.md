...
```
/kk:implement <natural language task OR plan path>
```
...
**Example:**
```
/kk:implement "Add user authentication to the app" --fast
/kk:implement path/to/plan.md --auto
/kk:implement "Refactor auth middleware" --tdd
```
...
- Route concrete bugs to `/kk:fix`; it frames intent first, then proves the root
...
- **Finalize (MANDATORY - never skip):**
  ...
  5. Run `/kk:journal` to write a concise technical journal entry upon completion — unless the shared "Journal step — opt-out" below applies.
...
| Scout | `kk:scout` | Optional in code |
...
**Typically follows:** `the engineer plan skill` (execute a plan), `/kk:brainstorm` (implement agreed solution)
**Typically precedes:** `the installed code-review skill` (review after implementation), `the installed test skill` (validate changes)
**Related:** `/kk:fix` (alternative for bug fixes), `the engineer plan skill` (create plan before cooking)
