...
Shared by every skill that creates, resolves, or mutates a plan (`kk:plan`,
`kk:implement`, and any other skill referencing this file).
...
- `ak plan use <plan-dir>` sets the current-plan pointer for this worktree so
  downstream tooling (`kk:implement`, dashboards, other agents) can resolve the
