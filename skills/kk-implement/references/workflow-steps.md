...
- Use `/kk:scout ext` or `scout` agent for codebase search
...
**Fast:**
- Use `/kk:plan --fast` with scout results only
- Minimal planning, focus on action

**Parallel:**
- Use `/kk:plan --parallel` for dependency graph + file ownership matrix
...
- Use `ask_user capability` to ask: "Validate the plan or approve plan to start implementation?" - "Validate" / "Approve" / "Abort" / "Other" ("Request revisions")
  - "Validate": run `/kk:validate-plan` skill invocation
...
- Use `kk:ai-multimodal` for image assets
