...
1. Activate `kk:scout` skill OR launch 2-3 parallel `Explore` subagents
...
2. Activate `kk:debug` skill (systematic-debugging + root-cause-tracing techniques).
3. Activate `kk:sequential-thinking` skill — form hypotheses through structured reasoning, NOT guessing.
4. Spawn parallel `Explore` subagents to test each hypothesis against codebase evidence.
5. If 2+ hypotheses fail → auto-activate `kk:problem-solving` skill for alternative approaches.
...
- For multiple viable repairs or an architecture decision, activate
  `kk:brainstorm`, compare trade-offs, and resolve the direction before
  implementation.
...
6. Run `/kk:journal` to write a concise technical journal entry upon completion — unless the shared "Journal step — opt-out" below applies.
...
Explicit `/kk:journal` and `ak journal create` are unaffected. The rest of the Finalize block above stays MANDATORY.
...
- `kk:scout` (Step 1) — understand before diagnosing
- `kk:debug` (Step 2) — systematic root cause investigation
- `kk:sequential-thinking` (Step 2) — structured hypothesis formation

**Always activate (Step 6 Finalize):**
- `kk:project-management` — MANDATORY for sync-back and progress tracking, every fix

**Conditional:**
- `kk:problem-solving` — auto-triggers when 2+ hypotheses fail in Step 2
- `kk:brainstorm` — after diagnosis when multiple valid approaches or an architecture decision remain
- `kk:context-engineering` — fixing AI/LLM/agent code
...
**Related:** `/kk:implement` (alternative for feature work), `the engineer debug skill` (diagnose before fixing)
