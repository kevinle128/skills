...
Activate `kk:debug` skill. Activate `kk:sequential-thinking` for structured analysis.
...
5. Run `/kk:journal` to log decisions — unless the shared "Journal step — opt-out" applies (see SKILL.md; skip when `--skip-journal` or `journal.auto=false`).
...
|------|------------------|
| 1 | `kk:scout` (minimal) or direct file read |
| 2 | `kk:debug`, `kk:sequential-thinking` |
| 3 | Parallel `run_shell` for verification |
| 4 | `code-reviewer` subagent |
| 5 | Report |
| 6 | `the engineer project-management skill` (MANDATORY), conditional `docs-manager`, `git-manager`, `/kk:journal` (unless the shared "Journal step — opt-out" applies — see SKILL.md) |

**Extra:** `kk:context-engineering` if dealing with AI/LLM code
