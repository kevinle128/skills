...
1. `kk:ui-ux-pro-max` for design-system and UX guidance
2. `kk:frontend-design` for implementation patterns

## Pre-Fix Research

```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/kk-ui-ux-pro-max/scripts/search.py "<product-type>" --domain product
python3 ${CLAUDE_PLUGIN_ROOT}/skills/kk-ui-ux-pro-max/scripts/search.py "<style>" --domain style
python3 ${CLAUDE_PLUGIN_ROOT}/skills/kk-ui-ux-pro-max/scripts/search.py "accessibility" --domain ux
```
...
Analyze screenshots or videos with `kk:ai-multimodal`. Read the project's
...
`kk:ai-multimodal` when useful. If incorrect, return to Step 2.

### Step 4: Inspect Runtime Behavior

Use `kk:agent-browser`, Chrome MCP / `chrome-devtools-mcp`, or project-native
...
- Use `kk:ai-multimodal` for generating visual assets
