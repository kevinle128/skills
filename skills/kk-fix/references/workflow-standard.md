...
1. Activate `kk:scout` skill OR launch 2-3 parallel `Explore` subagents when delegation is explicitly requested/permitted.
...
2. Activate `kk:debug` skill. Use `debugger` subagent if needed.
3. Activate `kk:sequential-thinking` — form hypotheses through structured reasoning.
4. Use delegated `Explore` subagents to test hypotheses only when delegation is explicitly requested/permitted.
5. If 2+ hypotheses fail → auto-activate `kk:problem-solving`.
...
approaches or an architecture decision remain, activate `kk:brainstorm` and
...
- Apply `kk:problem-solving` skill if stuck
- Use `kk:sequential-thinking` for complex logic
...
- Activate `kk:project-management` for task sync-back and plan status updates
...
- Run `/kk:journal` — unless the shared "Journal step — opt-out" applies (see SKILL.md; skip when `--skip-journal` or `journal.auto=false`)
...
|------|------------------|
| 1 | `kk:scout` OR parallel `Explore` subagents when delegation is permitted |
| 2 | `kk:debug`, `kk:sequential-thinking`, optional delegated debugger/Explore when permitted, (`kk:problem-solving` auto), conditional `kk:brainstorm` after diagnosis |
| 3 | `kk:problem-solving` (if stuck), `kk:sequential-thinking` (complex logic) |
...
| 6 | `kk:project-management`; docs/git delegation only when permitted |

**Rules:** Don't skip steps. Validate before proceeding. One phase at a time.
**Frontend:** Use `kk:agent-browser`, Chrome MCP / `chrome-devtools-mcp`, or any relevant project-native browser tests to verify.
**Visual Assets:** Use `kk:ai-multimodal` for visual assets generation, analysis and verification.
