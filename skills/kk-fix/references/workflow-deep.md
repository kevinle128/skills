...
**Mandatory:** Activate `kk:scout` skill or, when delegation is permitted,
...
2. Activate `kk:debug` skill (systematic-debugging + root-cause-tracing).
3. Activate `kk:sequential-thinking` — structured hypothesis formation.
...
Implement per plan. Use `kk:context-engineering`, `kk:sequential-thinking`, `kk:problem-solving`.
...
- Activate `kk:project-management` for task sync-back, plan status updates, and progress tracking
...
| 2 | `kk:debug`, `kk:sequential-thinking`, optional delegated Explore when permitted, (`kk:problem-solving` auto) |
| 3 | `researcher` via `delegate_agent` when permitted |
| 4 | `kk:brainstorm` |
| 5 | `planner` |
| 6 | `kk:problem-solving`, `kk:sequential-thinking`, `kk:context-engineering` |
...
| 9 | `kk:project-management`; docs/git delegation only when permitted |
