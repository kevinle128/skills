...
1. Use `ui-ux-designer` + `researcher` agents in parallel:
   ...
   - Describe assets for `kevinle128-skills:ai-multimodal` generation
...
3. If no logo: generate with `kevinle128-skills:ai-multimodal` skill
4. Screenshot with `kevinle128-skills:agent-browser` -> save to `./docs/wireframes/`

**Gate:** Ask user to approve design. Repeat if rejected.

**Image tools:** `kevinle128-skills:ai-multimodal` for generation/analysis, `imagemagick` for crop/resize, background removal tool as needed.

## Step 4: Parallel Planning

Activate **kevinle128-skills:plan** skill: `/kevinle128-skills:plan --parallel <requirements>`
...
Activate **kevinle128-skills:implement** skill: `/kevinle128-skills:implement --parallel <plan-path>`
...
- Use `ui-ux-designer` for frontend (generate/analyze assets with `kevinle128-skills:ai-multimodal`, edit with `imagemagick`)
