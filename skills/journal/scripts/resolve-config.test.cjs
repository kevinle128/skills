...
function makeProject() {
  const projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'kk-journal-resolve-config-'));
  fs.mkdirSync(path.join(projectRoot, '.agentkit'), { recursive: true });
  return projectRoot;
}

function makeHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'kk-journal-resolve-config-home-'));
}
