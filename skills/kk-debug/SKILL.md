...
Visual verification of frontend implementations via `kk:agent-browser`, `kk:chrome-profile`, Chrome MCP / `chrome-devtools-mcp`, or project-native browser tests. Use `kk:chrome-profile` and its exact tab binding when real Chrome profile state matters; raw Chrome MCP navigation is only for generic/profile-independent inspection. Detect if frontend-related -> check browser tool availability -> screenshot + console error check -> report. Skip if not frontend.
...
- **Codebase:** `kk:docs-seeker` skill for package/plugin docs; `kk:repomix` skill for codebase summary
- **Scouting:** `/kk:scout` or `/kk:scout ext` for finding relevant files
- **Frontend:** `kk:agent-browser`, `kk:chrome-profile`, Chrome MCP / `chrome-devtools-mcp`, or project-native browser tests for visual verification. For real profile state, let `chrome-profile open --json` create the tab and bind to its returned selector before using MCP inspection tools.
- **Skills:** Activate `kk:problem-solving` skill when stuck on complex issues
...
**Typically follows:** `/kk:scout` (after locating relevant code)
**Typically precedes:** `/kk:fix` (fix the diagnosed issue), `/kk:brainstorm` (explore solutions for complex problems)
**Related:** `/kk:scout` (discover before debugging), `/kk:fix` (fix after diagnosing)
