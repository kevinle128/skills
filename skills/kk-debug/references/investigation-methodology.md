...
- Use `/kk:scout` or `/kk:scout ext` to find relevant files whenever context is missing or conflicting
- Use `kk:repomix` only when a broad packaged snapshot materially improves the investigation; do not generate a mandatory summary document
- Use `kk:docs-seeker` skill for package/plugin documentation
