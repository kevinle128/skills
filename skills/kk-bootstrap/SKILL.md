...
```
/kk:bootstrap <user-requirements>
```
...
| Flag | Effect |
|------|--------|
| `--yagni` | Opt into YAGNI: challenge and cut scope not needed for the stated outcome (default: scaffold the full requested scope). Passed through to `kk:plan` and `kk:implement` |

**Example:**
```
/kk:bootstrap "Build a SaaS dashboard with auth" --fast
/kk:bootstrap "E-commerce platform with Stripe" --parallel
```
...
Activate **kk:implement** skill with the plan path and mode-appropriate flag:
- `--full` → `/kk:implement <plan-path>` (interactive review gates)
- `--auto` → `/kk:implement --auto <plan-path>` (explicit autonomous implementation)
- `--fast` → `/kk:implement <plan-path>` (skip extra research, keep cook review gates)
- `--parallel` → `/kk:implement --parallel <plan-path>` (multi-agent execution)
...
- Run `/kk:journal` to write a concise technical journal entry upon completion — unless the shared "Journal step — opt-out" below applies.
