---
name: validate-plan
description: "Validate an implementation plan against the real codebase, prove each feature through its production trigger and end-to-end test, resolve material decisions with the user, and reconcile the whole plan before implementation."
user-invocable: true
when_to_use: "Invoke after planning and before implementation when a plan needs evidence-backed validation."
category: utilities
keywords: [plan, validation, interview, verification, consistency, production-flow, end-to-end]
...
3. If no plan found → Ask user to specify path or run `/kevinle128-skills:plan --hard` first
...
### Step 2.5: Production Proof and Verification Pass
...
Run the `Mandatory Production Proof Gate` for every plan.
...
2. For each feature, identify the real actor and the existing or planned production trigger.
3. Trace the complete planned production path from that trigger through real internal components to an observable result.
...
5. Verify that the plan includes an end-to-end test which enters through the same production boundary.
...
7. Record one row per feature in the `Production Proof Matrix` defined in `references/verification-roles.md`.

Do not accept a helper-level, handler-level, repository-level, or component-only test as end-to-end proof when a higher production boundary exists.
One scenario can cover several features only when its steps and assertions prove each feature explicitly.

If neither the existing system nor the plan provides a production trigger, public surface, scheduler registration, event ingress, or owning caller for a required feature, stop and open a HITL question with `ask_user capability`.
...
The red-team guard never skips the Mandatory Production Proof Gate.
...
5. **Append results** to `## Validation Log` after the Production Proof Matrix:
   ```
...
Production-surface questions are mandatory and do not count against the configured question range.
...
- Focus on: missing production surfaces, assumptions, risks, tradeoffs, architecture
- Use a popup-capable `ask_user capability` call for every `NEEDS_DECISION` production proof row
...
- Production Proof Matrix and gate status
...
- Never waive a missing production trigger because the feature is internal or has no direct user interface
...
- Never recommend implementation until Production Proof passes and the whole-plan consistency sweep has no unresolved contradictions
