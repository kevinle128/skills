...
Activate `kevinle128-skills:debug` skill. Activate `kevinle128-skills:sequential-thinking` for structured analysis.
...
5. Run `/kevinle128-skills:journal` to log decisions — unless the shared "Journal step — opt-out" applies (see SKILL.md; skip when `--skip-journal` or `journal.auto=false`).
...
|------|------------------|
| 1 | `kevinle128-skills:scout` (minimal) or direct file read |
| 2 | `kevinle128-skills:debug`, `kevinle128-skills:sequential-thinking` |
| 3 | Parallel `run_shell` for verification |
| 4 | `code-reviewer` subagent |
| 5 | Report |
| 6 | `the engineer project-management skill` (MANDATORY), conditional `docs-manager`, `git-manager`, `/kevinle128-skills:journal` (unless the shared "Journal step — opt-out" applies — see SKILL.md) |

**Extra:** `kevinle128-skills:context-engineering` if dealing with AI/LLM code
