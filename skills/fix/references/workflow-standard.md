...
1. Activate `kevinle128-skills:scout` skill OR launch 2-3 parallel `Explore` subagents when delegation is explicitly requested/permitted.
...
2. Activate `kevinle128-skills:debug` skill. Use `debugger` subagent if needed.
3. Activate `kevinle128-skills:sequential-thinking` — form hypotheses through structured reasoning.
4. Use delegated `Explore` subagents to test hypotheses only when delegation is explicitly requested/permitted.
5. If 2+ hypotheses fail → auto-activate `kevinle128-skills:problem-solving`.
...
approaches or an architecture decision remain, activate `kevinle128-skills:brainstorm` and
...
- Apply `kevinle128-skills:problem-solving` skill if stuck
- Use `kevinle128-skills:sequential-thinking` for complex logic
...
- Activate `kevinle128-skills:project-management` for task sync-back and plan status updates
...
- Run `/kevinle128-skills:journal` — unless the shared "Journal step — opt-out" applies (see SKILL.md; skip when `--skip-journal` or `journal.auto=false`)
...
|------|------------------|
| 1 | `kevinle128-skills:scout` OR parallel `Explore` subagents when delegation is permitted |
| 2 | `kevinle128-skills:debug`, `kevinle128-skills:sequential-thinking`, optional delegated debugger/Explore when permitted, (`kevinle128-skills:problem-solving` auto), conditional `kevinle128-skills:brainstorm` after diagnosis |
| 3 | `kevinle128-skills:problem-solving` (if stuck), `kevinle128-skills:sequential-thinking` (complex logic) |
...
| 6 | `kevinle128-skills:project-management`; docs/git delegation only when permitted |

**Rules:** Don't skip steps. Validate before proceeding. One phase at a time.
**Frontend:** Use `kevinle128-skills:agent-browser`, Chrome MCP / `chrome-devtools-mcp`, or any relevant project-native browser tests to verify.
**Visual Assets:** Use `kevinle128-skills:ai-multimodal` for visual assets generation, analysis and verification.
