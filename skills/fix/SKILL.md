...
1. Activate `kevinle128-skills:scout` skill OR launch 2-3 parallel `Explore` subagents
...
2. Activate `kevinle128-skills:debug` skill (systematic-debugging + root-cause-tracing techniques).
3. Activate `kevinle128-skills:sequential-thinking` skill — form hypotheses through structured reasoning, NOT guessing.
4. Spawn parallel `Explore` subagents to test each hypothesis against codebase evidence.
5. If 2+ hypotheses fail → auto-activate `kevinle128-skills:problem-solving` skill for alternative approaches.
...
- For multiple viable repairs or an architecture decision, activate
  `kevinle128-skills:brainstorm`, compare trade-offs, and resolve the direction before
  implementation.
...
6. Run `/kevinle128-skills:journal` to write a concise technical journal entry upon completion — unless the shared "Journal step — opt-out" below applies.
...
Explicit `/kevinle128-skills:journal` and `ak journal create` are unaffected. The rest of the Finalize block above stays MANDATORY.
...
- `kevinle128-skills:scout` (Step 1) — understand before diagnosing
- `kevinle128-skills:debug` (Step 2) — systematic root cause investigation
- `kevinle128-skills:sequential-thinking` (Step 2) — structured hypothesis formation

**Always activate (Step 6 Finalize):**
- `kevinle128-skills:project-management` — MANDATORY for sync-back and progress tracking, every fix

**Conditional:**
- `kevinle128-skills:problem-solving` — auto-triggers when 2+ hypotheses fail in Step 2
- `kevinle128-skills:brainstorm` — after diagnosis when multiple valid approaches or an architecture decision remain
- `kevinle128-skills:context-engineering` — fixing AI/LLM/agent code
...
**Related:** `/kevinle128-skills:implement` (alternative for feature work), `the engineer debug skill` (diagnose before fixing)
