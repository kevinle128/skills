...
# Implement - Smart Feature Delivery
...
```
/kevinle128-skills:implement <natural language task OR plan path>
```
...
**Example:**
```
/kevinle128-skills:implement "Add user authentication to the app" --fast
/kevinle128-skills:implement path/to/plan.md --auto
/kevinle128-skills:implement "Refactor auth middleware" --tdd
```
...
- Route concrete bugs to `/kevinle128-skills:fix`; it frames intent first, then proves the root
...
- **Finalize (MANDATORY - never skip):**
  ...
  5. Run `/kevinle128-skills:journal` to write a concise technical journal entry upon completion — unless the shared "Journal step — opt-out" below applies.
...
| Scout | `kevinle128-skills:scout` | Optional in code |
...
**Typically follows:** `the engineer plan skill` (execute a plan), `/kevinle128-skills:brainstorm` (implement agreed solution)
**Typically precedes:** `the installed code-review skill` (review after implementation), `the installed test skill` (validate changes)
**Related:** `/kevinle128-skills:fix` (alternative for bug fixes), `the engineer plan skill` (create plan before cooking)
