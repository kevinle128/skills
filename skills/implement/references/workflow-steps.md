...
- Use `/kevinle128-skills:scout ext` or `scout` agent for codebase search
...
**Fast:**
- Use `/kevinle128-skills:plan --fast` with scout results only
- Minimal planning, focus on action

**Parallel:**
- Use `/kevinle128-skills:plan --parallel` for dependency graph + file ownership matrix
...
- Use `ask_user capability` to ask: "Validate the plan or approve plan to start implementation?" - "Validate" / "Approve" / "Abort" / "Other" ("Request revisions")
  - "Validate": run `/kevinle128-skills:validate-plan` skill invocation
...
- Use `kevinle128-skills:ai-multimodal` for image assets
