...
Shared by every skill that creates, resolves, or mutates a plan (`kevinle128-skills:plan`,
`kevinle128-skills:implement`, and any other skill referencing this file).
...
- `ak plan use <plan-dir>` sets the current-plan pointer for this worktree so
  downstream tooling (`kevinle128-skills:implement`, dashboards, other agents) can resolve the
...
- Publishing never overwrites the body of a pre-existing issue that supplied
  plan input; it only adds links, comments, or labels.
...
blind implementation during review-fix cycles, and the index close is a one-way
