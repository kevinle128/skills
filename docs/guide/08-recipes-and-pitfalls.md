...
```text
/kk:plan Add a public API that lets a user change the active model, thinking mode, and fast mode.
...
```text
/kk:plan Add model controls to the session settings page --hard --tdd
...
```text
/kk:validate-plan /absolute/path/to/plan.md
```
...
```text
/kk:fix Users receive duplicate notifications after a retry.
...
```text
/kk:debug Explain why an idle worker applies an older session generation after wake.
Do not change code.
...
```text
/kk:feature-lens owner/source-repository "retry strategy" --port
```

Feature Lens treats the source repository as untrusted evidence, maps the feature to local architecture, challenges incompatible assumptions, and hands the approved result to `kk:plan`.

## Execute Independent Phases in Parallel

```text
/kk:plan Add the API, admin UI, and audit export --parallel --tdd
```
...
```text
/kk:implement /absolute/path/to/plan.md --auto
...
```text
/kk:chrome-profile Use my work profile to open the settings page and confirm the saved model is shown.
```

Use `kk:agent-browser` instead when the test should start from a clean browser state.
...
```text
/kk:mermaidjs-v11 Draw the user request, commit, worker wake, dispatch, and callback validation sequence.
```

Use `kk:tech-graph` when the result must be a polished SVG and PNG artifact.

Use `kk:preview` when you want an interactive explanation or visual review.
...
Stop during planning and resolve the missing runtime surface instead of inventing a test-only path.
...
KevinKit skills use the `kk:*` namespace.
...
Back to [The KevinKit Guide](./README.md).
